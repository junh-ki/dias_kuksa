...
def storeNewMetrics(list, db, metrics):
	# Check the last data
	lastVal = getLastMetrics(db, metrics)
...
while True:
	storeNewMetrics(nox_list, db, "cumulative_nox")
	storeNewMetrics(bin_pos_list, db, "bin_position")
	storeNewMetrics(work_list, db, "cumulative_work")
	time.sleep(1)
